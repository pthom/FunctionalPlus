...
echo "Xvfb -screen 0 1280x1024x16 & sleep 1  &&  export DISPLAY=:0  &&  icewm & x11vnc -forever" > /start_x_vnc.sh
chmod +x /start_x_vnc.sh
echo "/start_x_vnc.sh > /dev/null 2>&1 &" >> /start_x_vnc_quiet.sh
chmod +x /start_x_vnc_quiet.sh
